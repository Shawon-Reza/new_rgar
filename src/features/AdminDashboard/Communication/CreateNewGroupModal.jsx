...
const dropdownButtonClass = "flex h-11 w-full items-center justify-between gap-3 rounded-xl border border-[#d9e1ec] bg-white px-4 text-left text-sm font-extrabold text-[#172033] shadow-sm outline-none transition hover:border-[#2f6ff3] focus:border-[#2f6ff3] focus:ring-4 focus:ring-blue-100 disabled:cursor-not-allowed disabled:bg-[#f6f8fb] disabled:text-[#9aa8bd]";
...
const CreateNewGroupModal = ({ onClose }) => {
    ...
    const SelectMenu = ({ id, valueLabel, placeholder, options, disabled, onSelect }) => {
                        ...
                        {options.length > 0 ? (
                            options.map((option) => (
                                <button
                                    ...
                                    className={`flex min-h-10 w-full items-center justify-between gap-3 rounded-xl px-3 text-left text-sm font-bold transition ${option.active
            ...
            <form className="space-y-4">
                ...
                <div>
                    ...
                    <SelectMenu
                        ...
                        valueLabel={selectedClinic?.name}
                        options={(userList?.my_clinics || []).map((clinic) => ({
                            value: String(clinic.id),
                            label: clinic.name,
                            raw: clinic,
                            active: selectedClinic?.id === clinic.id,
                        }))}
                        onSelect={(value) => handleClinicChange(value)}
                    />
                </div>
                {/* Role & Add Members */}
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="flex-1">
                        ...
                        <SelectMenu
                            ...
                            valueLabel={selectedRole ? selectedRole.replace("_", " ") : ""}
                    ...
                    <div className="flex-1">
                        ...
                        <SelectMenu
                            id="member"
                            placeholder={selectedClinic ? 'Select a member' : 'Select a clinic first'}
                            valueLabel=""
                            ...
                            onSelect={(value, member) => {
                                const selectedMember = member || filteredMembers.find(m => m.id === parseInt(value));
