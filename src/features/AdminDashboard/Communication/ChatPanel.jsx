...
import { FiChevronLeft, FiSend, FiInfo, FiPaperclip, FiX, FiPlus, FiList } from "react-icons/fi";
...
const buildNewCaseSeparator = (separatorData = {}) => ({
  id: separatorData.id || `newcase-${separatorData.room_id || "room"}-${Date.now()}`,
...
const ChatPanel = ({ chatRoom, roomType, activeTab, forwardedMessage, onForwardConsumed, avatar, avatarNode, draftMessage, onDraftConsumed, onBackToList, onOpenTopics }) => {
  ...
  const isModernChatSurface = isChartingPage || isAssistancePage || isTeamChatPage;
  ...
  useEffect(() => {
    ...
    const socket = connectWebSocketForChat({
      ...
      onMessage: (payload) => {
        ...
        const cacheMessage = isNewCaseSeparator ? buildNewCaseSeparator(newMessage) : newMessage;
  ...
  const sendMessage = useCallback(async ({ content, files = [], suppressAiTyping = false }) => {
    ...
    try {
      ...
      const messageType = path === "charting-ai"
        ? (chartingMode === "chart" ? "chart" : "chart")
        : "text";
      ...
      const isNewCaseSeparator = responseData?.type === "newcase_separator" || responseData?.is_separator;
  ...
  const handleStarterQuestionClick = useCallback(async (question) => {
  ...
  return (
    <div className={isModernChatSurface ? "flex h-full min-h-0 flex-col overflow-hidden bg-[#eef2f8]" : "flex flex-col h-full border border-gray-300 rounded-lg bg-white/50 overflow-hidden"}>
      {!chatRoom ? (
        ...
        <>
            ...
            <header className="flex min-h-[64px] items-center justify-between gap-3 bg-[#172640] px-3 py-2 text-white shadow-sm md:gap-4 md:px-5 md:py-3">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="hidden text-[11px] font-semibold uppercase tracking-[0.28em] text-[#9dc0ff] md:block">Physician Workstation</p>
                  <h1 className="truncate text-[13px] font-semibold leading-5 text-white md:text-base">Chartly</h1>
                </div>
              </div>
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <div className="hidden h-8 items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-4 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#9fb0c8] xl:flex">
                  <FiInfo size={14} />
                  Decision Support - Not Diagnostic - Physician Judgment Required
                </div>
              </div>
            </header>
          ...
          <header className="flex min-h-[56px] items-center justify-between gap-4 border-b border-[#dfe3ea] bg-white px-5">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#2B76F4]">AI Assistant</p>
              <div className="mt-0.5 flex items-center gap-1.5">
                <h1 className="text-base font-semibold leading-5 text-[#111827]">KyroAI</h1>
          ...
          <header className={`flex min-h-[64px] items-center justify-between gap-3 border-b border-[#e7ecf3] bg-white px-4 py-2 text-[#101827] shadow-none md:min-h-[70px] md:border-b-0 md:px-5 md:py-3 md:text-white md:shadow-sm ${roomType === "group" ? "md:bg-[#9f171d]" : "md:bg-[#172640]"}`}>
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <h1 className="truncate text-[14px] font-semibold leading-5 text-[#050b18] md:text-base md:text-white">{safeUser.name}</h1>
              </div>
            </div>
            ...
            <div className="flex shrink-0 items-center gap-3">
              ...
              <div ref={actionsDropdownRef} className={`relative hidden md:block ${data?.pages[0]?.room?.type === "ai" ? "md:hidden" : ""} `}>
                <button
                  type="button"
          ...
          <MessageList
            ...
            path={path}
            starterQuestions={isChartingPage ? CHARTING_STARTER_QUESTIONS : isAssistancePage ? EMPTY_ARRAY : roomType === "ai" ? AI_ASSISTANT_STARTER_QUESTIONS : EMPTY_ARRAY}
            onStarterQuestionClick={handleStarterQuestionClick}
          />

          {/* ........................................................Input Area For send text................................................ */}
          <div className={isModernChatSurface ? `border-t border-[#dfe5ef] px-3 py-2.5 shadow-[0_-8px_22px_rgba(15,23,42,0.04)] md:bg-white/95 md:px-4 md:py-3 ${isChartingPage ? "bg-white" : "bg-[#f7faff]"}` : "p-2 sm:p-4 border-t border-gray-300"}>
            ...
            <div className="flex items-center gap-2 sm:gap-3 w-full min-w-0">
              ...
              {(roomType === "group" || roomType === "private" || (roomType === "ai" && !isAssistancePage)) && (
                <>
                  <input
                    ref={fileInputRef}
                    type="file"
              ...
              {roomType === "group" ? (
                <div className="flex-1 relative min-w-0 ">
                  <MentionsInput
                    className={isTeamChatPage ? "mentions mentions--team" : "mentions mentions--multiLine"}
                ...
                <textarea
                  ...
                  onKeyDown={handleInputKeyDown}
                  placeholder={inputPlaceholder}
                  className={`flex-1 min-w-0 resize-none overflow-y-auto border outline-none ${isModernChatSurface
                  ...
                  style={isModernChatSurface ? { minHeight: isChartingPage ? "42px" : "40px", maxHeight: "120px" } : { minHeight: "70px", maxHeight: "220px" }}
                />
              )}
              <button
                onClick={handleSendMessage}
                disabled={(!inputMessage.trim() && attachments.length === 0) || isInputDisabled || (isAiRoom && isAiTyping)}
                className={isModernChatSurface
