...
import { FiThumbsUp, FiThumbsDown, FiDownload, FiFile, FiChevronDown, FiChevronRight, FiCornerUpRight, FiMessageCircle } from "react-icons/fi";
...
const getMessageKey = (message, index) =>
    message?.id || `${message?.type || "message"}-${message?.room_id || "room"}-${message?.created_at || index}`;
...
const MessageBubble = memo(({
    ...
    const isChartingMessage = path === "charting-ai" || roomType === "ai_charting";
    ...
    const aiDisplayName = path === "charting-ai" ? "Chartly AI" : path === "assistance" ? "KyroAI" : "AI Assistant";
    ...
    const bubbleClassName = isChartingResponse
        ...
        : isChartingMessage
        ? `px-4 py-3 rounded-2xl max-w-[340px] md:max-w-4xl break-words shadow-[0_3px_9px_rgba(15,23,42,0.12)] ${isAI
        ...
        : isTeamMessage
        ? `max-w-[275px] break-words rounded-2xl px-4 py-3 text-[14px] shadow-[0_2px_7px_rgba(15,23,42,0.12)] md:max-w-md ${isMe
    ...
    return (
        <div id={`message-${msg.id}`} className={`group flex ${isChartingResponse ? "mb-6" : isTeamMessage ? "mb-7" : "mb-4"} ${isMe ? "justify-end" : "justify-start"} ${isHighlighted ? 'animate-pulse' : ''} `}>
            {!isMe && (
                <div
                    className={isChartingResponse
                ...
                >
                    {isAI ? (
                        ...
                        <span className={`text-xs font-semibold ${isTeamMessage ? "text-white md:text-gray-600" : "text-gray-600"}`}>
            ...
            <div className={`flex flex-col ${isChartingResponse ? "min-w-0 flex-1" : ""} ${isMe ? "items-end" : "items-start"}`}>
                {(!isTeamMessage || !isMe) && !(isAssistanceMessage && isMe) && (
                    <div className={isChartingResponse
                ...
                {(isTeamMessage || (isAssistanceMessage && isMe)) && (
                    <div className={`mt-1 px-1 text-[10px] font-medium ${isAssistanceMessage && isMe ? "text-[#9c8db5]" : "text-[#8da0ba]"} ${isMe ? "self-end text-right" : "self-start text-left"}`}>
                        {messageTime}
                    </div>
                )}
            </div>
        ...
        >
            ...
            {groupedMessages.length === 0 ? (
                isAssistanceAI ? (
                    <div className="flex h-full min-h-[520px] flex-col items-center justify-center">
                        <div className="mx-auto mb-6 max-w-[360px] text-center">
                            ...
                            <h2 className="text-base font-semibold text-[#111827]">Ask KyroAI any questions</h2>
                            <p className="mt-2 text-sm font-medium leading-6 text-[#8b98ad]">
                                Type your question, or tap <span className="font-semibold text-[#2B76F4]">Topics</span> to browse by department.
                            </p>
                        </div>
                        <div className="w-full max-w-[390px] space-y-2">
                            {starterQuestions.map((question) => (
                                ...
                                >
                                    ...
                                    <FiChevronRight className="shrink-0 text-[#b9c6d6] transition group-hover:text-[#2B76F4]" size={18} />
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex h-full min-h-[520px] flex-col items-center pt-6 md:justify-center md:pt-0">
                        <div className="mb-5 text-center text-[11px] font-semibold uppercase tracking-[0.14em] text-[#2B76F4] md:hidden">
                            New Case - {new Date().toLocaleTimeString([], {
                                hour: "2-digit",
                                minute: "2-digit",
                            })}
                        </div>
                        <div className="mx-auto mb-5 hidden max-w-[300px] text-center md:block">
                            ...
                            <h2 className="text-base font-semibold text-[#284058]">Charting Assistant</h2>
                            <p className="mt-2 text-sm leading-5 text-[#8d9ab3]">
                                Ask me questions or use the action buttons above to generate charts and manage cases
                            </p>
                        </div>
                        <div className="w-full space-y-2">
                            {starterQuestions.map((question) => (
                                ...
                                >
                                    ...
                                    <FiChevronRight className="shrink-0 text-[#b9c6d6] transition-colors group-hover:text-[#2B76F4]" size={18} />
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex h-full min-h-[360px] items-center justify-center px-4 text-center">
                        <div className="max-w-[320px]">
                            ...
                            <h3 className="text-base font-semibold text-[#172033]">No messages yet</h3>
                            <p className="mt-2 text-sm font-medium leading-6 text-[#8da0ba]">Send a message to start this conversation.</p>
                        </div>
                    </div>
                ) : (
                <div className="flex h-full min-h-[220px] items-end justify-center px-2 pb-8 pt-6 sm:px-6 sm:pb-12">
                    {starterQuestions.length > 0 ? (
                        <div className="w-full max-w-4xl">
                            <div className="mb-5 flex items-center gap-3">
                                ...
                                <div className="text-left">
                                    <h3 className="text-base font-semibold text-gray-900">Start with AI Assistant</h3>
                                    <p className="text-sm text-gray-500">Choose a question or type your own message below.</p>
                                </div>
                            </div>
                        ...
                        <p className="text-gray-500">Start messaging to begin the conversation.</p>
                    )}
                </div>
                )
            ) : (
                groupedMessages.map((item, index) => {
                    if (item.type === 'date') {
                        ...
                        if (isNewCaseSeparator(item.data)) {
                            return <NewCaseSeparator key={getMessageKey(item.data, index)} item={item.data} isChartingAI={isChartingAI} />;
                        }
            ...
            {isAiTyping && (roomType === "ai" || path === "charting-ai") && (
                <div className="flex mb-4 items-start gap-2">
                    <figure>
                        <img src={aiAvater} alt="" className={`h-8 w-8 min-w-8 border ${isAssistanceAI || isChartingAI ? "rounded-xl border-[#dbe6f6]" : "rounded-full border-purple-200"}`} />
                    </figure>
                    <div className={`max-w-xs rounded-lg border px-4 py-2 ${isAssistanceAI || isChartingAI ? "border-[#e3e9f2] bg-white shadow-sm" : "border-purple-300 bg-purple-100"}`}>
                        <div className={`mb-1 text-xs font-semibold ${isAssistanceAI || isChartingAI ? "text-[#2B76F4]" : "text-purple-600"}`}>
                            {path === "charting-ai" ? "Chartly AI" : path === "assistance" ? "KyroAI" : "AI Assistant"}
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-sm text-gray-600">AI is thinking</span>
                            <div className="flex gap-1 ml-1">
                                <span className={`h-1.5 w-1.5 animate-bounce rounded-full ${isAssistanceAI || isChartingAI ? "bg-[#2B76F4]" : "bg-purple-600"}`} style={{ animationDelay: '0ms' }}></span>
                                <span className={`h-1.5 w-1.5 animate-bounce rounded-full ${isAssistanceAI || isChartingAI ? "bg-[#2B76F4]" : "bg-purple-600"}`} style={{ animationDelay: '150ms' }}></span>
                                <span className={`h-1.5 w-1.5 animate-bounce rounded-full ${isAssistanceAI || isChartingAI ? "bg-[#2B76F4]" : "bg-purple-600"}`} style={{ animationDelay: '300ms' }}></span>
                            </div>
                        </div>
                    </div>
                </div>
